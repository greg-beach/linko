...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	if longURL == "" {
		httpError(r.Context(), w, http.StatusBadRequest, fmt.Errorf("missing url paramter"))
		return
	}
	u, err := url.Parse(longURL)
	if err != nil || u.Scheme == "" || u.Host == "" {
		httpError(r.Context(), w, http.StatusBadRequest, fmt.Errorf("invalid URL: must include scheme (http/https)"))
		return
	}
